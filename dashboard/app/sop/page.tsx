...
export default function SopKelolaanPage() {
  ...
  // 🔥 FIX BARU: Interseptor Tombol Enter untuk mempercepat proses edit
  const handleKeyDownEnter = (e: React.KeyboardEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      e.currentTarget.blur(); // Memicu onBlur otomatis untuk save data ke BE
    }
  };

  return (
    <div className="max-w-[1600px] mx-auto space-y-6 py-4 px-2 text-[#1D1D1F]">
      {/* Header Halaman */}
      <div className="border-b border-[#E5E5EA] pb-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Standard Operating Procedure (SOP)</h1>
          <p className="text-sm text-[#86868B] mt-1 font-medium">Panduan master manajemen terpadu proses operasional internal PT PIPOSMART DIGITAL INDONESIA.</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {activeTab === "klasifikasi" ? (
            <>
              <button onClick={() => handleAddItem("potensi")} className="px-3 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-xl font-bold text-xs hover:bg-emerald-100 transition shadow-sm">➕ Potensi</button>
              <button onClick={() => handleAddItem("wajib-bisnis")} className="px-3 py-1.5 bg-blue-50 text-blue-700 border border-blue-200 rounded-xl font-bold text-xs hover:bg-blue-100 transition shadow-sm">➕ Wajib Bisnis</button>
              <button onClick={() => handleAddItem("tidak-potensi")} className="px-3 py-1.5 bg-rose-50 text-rose-700 border border-rose-200 rounded-xl font-bold text-xs hover:bg-rose-100 transition shadow-sm">➕ Tidak Potensi</button>
              <button onClick={() => handleAddItem("todo-sales")} className="px-3 py-1.5 bg-amber-50 text-amber-800 border border-amber-200 rounded-xl font-bold text-xs hover:bg-amber-100 transition shadow-sm">➕ Task Sales</button>
              <button onClick={() => handleAddItem("todo-cs")} className="px-3 py-1.5 bg-indigo-50 text-indigo-700 border border-indigo-200 rounded-xl font-bold text-xs hover:bg-indigo-100 transition shadow-sm">➕ Task CS</button>
            </>
          ) : (
            <>
              <button onClick={() => handleAddItem("call")} className="px-3 py-1.5 bg-blue-50 text-[#007AFF] border border-blue-200 rounded-xl font-bold text-xs hover:bg-blue-100 transition shadow-sm">➕ Modul Call</button>
              <button onClick={() => handleAddItem("chat")} className="px-3 py-1.5 bg-purple-50 text-purple-700 border border-purple-200 rounded-xl font-bold text-xs hover:bg-purple-100 transition shadow-sm">➕ Modul Chat</button>
              <button onClick={() => handleAddItem("reason")} className="px-3 py-1.5 bg-gray-100 text-gray-700 border border-gray-200 rounded-xl font-bold text-xs hover:bg-gray-200 transition shadow-sm">➕ Reason Block</button>
            </>
          )}
        </div>
      </div>
      ...
      {/* Tab Navigasi Utama */}
      <div className="flex bg-[#F5F5F7] p-1 rounded-xl w-fit border border-[#E8E8ED]">
        <button
          onClick={() => { setActiveTab("klasifikasi"); setEditingKey(null); }}
          className={`px-4 py-1.5 text-[13px] font-semibold rounded-lg transition-all ${activeTab === "klasifikasi" ? "bg-white text-[#007AFF] shadow-sm" : "text-[#86868B] hover:text-[#1D1D1F]"}`}
        >
          Klasifikasi Nasabah & To Do List
        </button>
        <button
          onClick={() => { setActiveTab("modul"); setEditingKey(null); }}
          className={`px-4 py-1.5 text-[13px] font-semibold rounded-lg transition-all ${activeTab === "modul" ? "bg-white text-[#007AFF] shadow-sm" : "text-[#86868B] hover:text-[#1D1D1F]"}`}
        >
          Modul Call & Chat CS
        </button>
      </div>
      ...
      {loading ? (
        ...
        <>
          {/* =========================================================
              🔴 TAB 1: KLASIFIKASI NASABAH & TO DO LIST (CO-EXIST GRID)
             ========================================================= */}
          {activeTab === "klasifikasi" && (
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
              
              {/* KOLOM KIRI (7/12 Width): KELOLAAN KLASIFIKASI NASABAH */}
              <div className="xl:col-span-7 space-y-6">
                
                {/* Card Nasabah Potensi */}
                <div className="bg-white rounded-2xl border border-[#E8E8ED] shadow-sm overflow-hidden">
                  <div className="bg-[#E2F7ED] px-6 py-4 border-b border-[#D1F2E2]">
                    <h2 className="text-base font-bold text-[#15803D]">Kategori Nasabah Potensi</h2>
                  </div>
                  <div className="p-6 space-y-4">
                    <p className="text-[13px] text-[#48484A] font-medium leading-relaxed">Nasabah dapat dikategorikan sebagai **Nasabah Potensi** apabila memenuhi indikator berikut:</p>
                        ...
                        return (
                          <li key={item.id} className="flex items-start justify-between gap-2.5 group/item p-1 hover:bg-gray-50 rounded-lg transition">
                            <div className="flex items-start gap-2.5 flex-1">
                              <span className="text-[#15803D] mt-0.5 shrink-0">{item.label}</span>
                              {editingKey === key ? (
                                <input type="text" value={item.keterangan} onChange={(e) => { const u = [...indikatorPotensi]; u[index].keterangan = e.target.value; setIndikatorPotensi(u); }} onBlur={() => { setEditingKey(null); handleLiveUpdateBackend(item.id, "keterangan", item.keterangan); }} onKeyDown={handleKeyDownEnter} autoFocus className="w-full border px-2 py-0.5 rounded focus:outline-none" />
                              ) : (
                                <span onClick={() => setEditingKey(key)} className="cursor-pointer flex-1">{item.keterangan}</span>
                              )}
                            </div>
                            {item.id > 6 && <button onClick={() => handleDeleteItem(item.id)} className="text-gray-300 hover:text-rose-500 font-bold text-xs md:opacity-0 group-hover/item:opacity-100 transition px-1">✕</button>}
                          </li>
                        );
                      })}
                    </ul>
                    
                    <div className="bg-[#F5F5F7] p-4 rounded-xl border border-[#E8E8ED] mt-4">
                      <span className="text-[11px] font-bold text-[#007AFF] uppercase block mb-2">Untuk nasabah potensi, tim bisnis wajib:</span>
                      <ul className="space-y-1.5 text-[12px] text-[#48484A] font-medium leading-relaxed">
                          ...
                          return (
                            <li key={item.id} className="flex items-start justify-between gap-2 group/wajib">
                              <div className="flex items-start gap-2 flex-1">
                                ...
                                {editingKey === key ? (
                                  <input type="text" value={item.keterangan} onChange={(e) => { const u = [...kewajibanBisnis]; u[index].keterangan = e.target.value; setKewajibanBisnis(u); }} onBlur={() => { setEditingKey(null); handleLiveUpdateBackend(item.id, "keterangan", item.keterangan); }} onKeyDown={handleKeyDownEnter} autoFocus className="w-full border px-1 rounded focus:outline-none" />
                                ) : (
                                  <span onClick={() => setEditingKey(key)} className="cursor-pointer flex-1">{item.keterangan}</span>
                                )}
                              </div>
                              {item.id > 103 && <button onClick={() => handleDeleteItem(item.id)} className="text-gray-300 hover:text-rose-500 text-[10px] md:opacity-0 group-hover/wajib:opacity-100 transition px-1">✕</button>}
                            </li>
                          );
                        })}
                      </ul>
                ...
                <div className="bg-white rounded-2xl border border-[#E8E8ED] shadow-sm overflow-hidden">
                  <div className="bg-[#FEF2F2] px-6 py-4 border-b border-[#FEE2E2]">
                    <h2 className="text-base font-bold text-[#DC2626]">Kategori Nasabah Tidak Potensi (Ditarik)</h2>
                  </div>
                  <div className="p-6 space-y-4">
                    <p className="text-[13px] text-[#48484A] font-medium leading-relaxed">Nasabah ditarik dari daftar aktif apabila memenuhi kriteria berikut:</p>
                    <ul className="space-y-2.5 text-[13px] font-medium text-gray-700">
                      {indikatorTidakPotensi.map((item, index) => {
                        ...
                        return (
                          <li key={item.id} className="flex items-start justify-between gap-2.5 group/item p-1 hover:bg-gray-50 rounded-lg transition">
                            <div className="flex items-start gap-2.5 flex-1">
                              <span className="text-[#DC2626] mt-0.5 shrink-0">{item.label}</span>
                              {editingKey === key ? (
                            ...
                            {item.id > 13 && <button onClick={() => handleDeleteItem(item.id)} className="text-gray-300 hover:text-rose-500 font-bold text-xs md:opacity-0 group-hover/item:opacity-100 transition px-1">✕</button>}
                          </li>
                        );
                      })}
                    </ul>
              ...
              {/* KOLOM KANAN (5/12 Width): ✨ TO DO LIST SALES & CS */}
              <div className="xl:col-span-5 bg-white border border-[#E8E8ED] rounded-3xl shadow-sm overflow-hidden p-6 space-y-5">
                <div className="text-center bg-amber-50 border border-amber-200/60 p-3 rounded-2xl">
                  <h3 className="text-sm font-black text-amber-900 tracking-wide uppercase flex items-center justify-center gap-1.5">✨ TO DO LIST SALES & CS</h3>
                </div>

                {/* Seksi Team Sales */}
                <div className="space-y-3">
                  <h4 className="text-xs font-black text-gray-800 flex items-center gap-1.5 bg-gray-50 px-3 py-2 rounded-xl border border-gray-100">🔥 TEAM SALES</h4>
                  <div className="divide-y divide-gray-100 pl-1">
                    {todoSales.map((item, index) => {
                      const keyDesc = `sales-todo-${index}`;
                      return (
                        <div key={item.id} className="py-2.5 flex items-start justify-between gap-3 group/row text-[12.5px] font-medium text-gray-600 hover:bg-gray-50/50 rounded-lg transition px-1">
                          <div className="flex items-start gap-2.5 flex-1">
                            <span className="font-bold text-blue-600 shrink-0">{item.label}</span>
                            {editingKey === keyDesc ? (
                              <textarea value={item.keterangan} onChange={(e) => { const u = [...todoSales]; u[index].keterangan = e.target.value; setTodoSales(u); }} onBlur={() => { setEditingKey(null); handleLiveUpdateBackend(item.id, "keterangan", item.keterangan); }} onKeyDown={handleKeyDownEnter} autoFocus className="w-full border px-2 py-0.5 rounded text-xs focus:outline-none bg-white resize-none" rows={2} />
                            ) : (
                              <span onClick={() => setEditingKey(keyDesc)} className="cursor-pointer flex-1 leading-relaxed">{item.keterangan}</span>
                            )}
                          </div>
                          {item.id > 24 && <button onClick={() => handleDeleteItem(item.id)} className="text-gray-300 hover:text-rose-500 font-bold text-[11px] md:opacity-0 group-hover/row:opacity-100 transition px-1">✕</button>}
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="space-y-3 pt-2">
                  <h4 className="text-xs font-black text-gray-800 flex items-center gap-1.5 bg-gray-50 px-3 py-2 rounded-xl border border-gray-100">💬 TEAM CS</h4>
                  <div className="divide-y divide-gray-100 pl-1">
                    {todoCs.map((item, index) => {
                      ...
                      return (
                        <div key={item.id} className="py-2.5 flex items-start justify-between gap-3 group/row text-[12.5px] font-medium text-gray-600 hover:bg-gray-50/50 rounded-lg transition px-1">
                          ...
                          {item.id > 27 && <button onClick={() => handleDeleteItem(item.id)} className="text-gray-300 hover:text-rose-500 font-bold text-[11px] md:opacity-0 group-hover/row:opacity-100 transition px-1">✕</button>}
                        </div>
                      );
                    })}
                  </div>
          ...
          {activeTab === "modul" && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Modul Call */}
                <div className="bg-white rounded-2xl border border-[#E8E8ED] shadow-sm overflow-hidden">
                  <div className="px-6 py-4 bg-[#F5F5F7] border-b border-[#E8E8ED]">
                    <h3 className="text-base font-bold text-[#1D1D1F]">Modul Call CS</h3>
                  </div>
                  <div className="divide-y divide-[#F2F2F7]">
                    {modulCall.map((item, index) => {
                      ...
                      return (
                        <div key={item.id} className="p-4 flex flex-col sm:flex-row items-start justify-between gap-4 group/row hover:bg-gray-50/50 transition">
                          <div className="flex flex-col sm:flex-row gap-2 sm:gap-6 text-[13px] flex-1 w-full">
                            <div className="sm:w-1/3 shrink-0 font-bold text-[#007AFF]">
                              {editingKey === keyLabel ? (
                                <input type="text" value={item.label} onChange={(e) => { const u = [...modulCall]; u[index].label = e.target.value; setModulCall(u); }} onBlur={() => { setEditingKey(null); handleLiveUpdateBackend(item.id, "label", item.label); }} onKeyDown={handleKeyDownEnter} autoFocus className="w-full border px-1 rounded text-xs focus:outline-none" />
                              ) : (
                                <span onClick={() => setEditingKey(keyLabel)} className="cursor-pointer hover:bg-amber-50 rounded px-0.5">{item.label}</span>
                              )}
                            </div>
                            <div className="text-[#48484A] font-medium leading-relaxed flex-1">
                              {editingKey === keyDesc ? (
                                <textarea value={item.keterangan} onChange={(e) => { const u = [...modulCall]; u[index].keterangan = e.target.value; setModulCall(u); }} onBlur={() => { setEditingKey(null); handleLiveUpdateBackend(item.id, "keterangan", item.keterangan); }} onKeyDown={handleKeyDownEnter} autoFocus className="w-full border px-1 rounded resize-none text-xs focus:outline-none" rows={2} />
                              ) : (
                                <span onClick={() => setEditingKey(keyDesc)} className="cursor-pointer hover:bg-amber-50 rounded px-0.5">{item.keterangan}</span>
                              )}
                            </div>
                          </div>
                          {item.id > 34 && <button onClick={() => handleDeleteItem(item.id)} className="text-gray-300 hover:text-rose-500 font-bold text-xs md:opacity-0 group-hover/row:opacity-100 transition sm:mt-0.5 px-1">✕</button>}
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="bg-white rounded-2xl border border-[#E8E8ED] shadow-sm overflow-hidden">
                  <div className="px-6 py-4 bg-[#F5F5F7] border-b border-[#E8E8ED]">
                    <h3 className="text-base font-bold text-[#1D1D1F]">Modul Chat CS</h3>
                  </div>
                  <div className="divide-y divide-[#F2F2F7]">
                      ...
                      return (
                        <div key={item.id} className="p-4 flex flex-col sm:flex-row items-start justify-between gap-4 group/row hover:bg-gray-50/50 transition">
                          <div className="flex flex-col sm:flex-row gap-2 sm:gap-6 text-[13px] flex-1 w-full">
                            <div className="sm:w-1/3 shrink-0 font-bold text-[#007AFF]">
                              {editingKey === keyLabel ? (
                                <input type="text" value={item.label} onChange={(e) => { const u = [...modulChat]; u[index].label = e.target.value; setModulChat(u); }} onBlur={() => { setEditingKey(null); handleLiveUpdateBackend(item.id, "label", item.label); }} onKeyDown={handleKeyDownEnter} autoFocus className="w-full border px-1 rounded text-xs focus:outline-none" />
                              ) : (
                                <span onClick={() => setEditingKey(keyLabel)} className="cursor-pointer hover:bg-amber-50 rounded px-0.5">{item.label}</span>
                              )}
                            </div>
                          ...
                          {item.id > 41 && <button onClick={() => handleDeleteItem(item.id)} className="text-gray-300 hover:text-rose-500 font-bold text-xs md:opacity-0 group-hover/row:opacity-100 transition sm:mt-0.5 px-1">✕</button>}
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className="bg-white rounded-2xl border border-[#E8E8ED] shadow-sm p-6">
                <h3 className="text-base font-bold text-[#DC2626] flex items-center gap-2 mb-3"><span>⚠</span> Alasan Utama Label "No Call / No Chat" (Reason)</h3>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {reasons.map((item, index) => {
                    const keyReason = `reason-${index}`;
                    return (
                      <div key={item.id} className="bg-[#F5F5F7] p-4 rounded-xl border border-[#E8E8ED] text-center relative group/reason">
                        {item.id > 44 && <button onClick={() => handleDeleteItem(item.id)} className="absolute top-2 right-3 text-gray-300 hover:text-rose-500 font-black text-[10px] md:opacity-0 group-hover/reason:opacity-100 transition">✕</button>}
                        <span className="text-lg font-bold text-[#1D1D1F] block">{item.label}</span>
                        {editingKey === keyReason ? (
                          <input type="text" value={item.keterangan} onChange={(e) => { const u = [...reasons]; u[index].keterangan = e.target.value; setReasons(u); }} onBlur={() => { setEditingKey(null); handleLiveUpdateBackend(item.id, "keterangan", item.keterangan); }} onKeyDown={handleKeyDownEnter} autoFocus className="w-full bg-white border px-1 py-0.5 rounded text-xs font-semibold text-center mt-1 outline-none" />
                        ) : (
                          <span onClick={() => setEditingKey(keyReason)} className="text-[12px] text-[#48484A] font-semibold mt-1 block cursor-pointer hover:bg-amber-50 rounded p-0.5">{item.keterangan}</span>
                        )}
                      </div>
                    );
                  })}
